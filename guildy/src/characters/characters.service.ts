...
export class CharactersService {
  ...
  async create(username : string , avatar : string | null ,discordId: string, guildId: string, dto: CreateCharacterDto) {
    try {
 ...
 async getGuilds(discordId : string) {
    const user = await this.db.user.findUnique({
      where : {discordId},
      include : {
        guildMembers : {include : {
          guild : true  ,
        }}
      }
    })
